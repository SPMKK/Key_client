...
permanent_failure_keywords = ["INVALID_ARGUMENT","suspended", "api key expired", "permission_denied","CONSUMER_SUSPENDED", "Permission denied", 'API_KEY_INVALID', "expired", 'UNAVAILABLE', "	PERMISSION_DENIED"]
# ==============================================================================
...
class GeneratorGemini:
    def __init__(self, api_key_manager: 'GeminiApiKeyManager', model_name="gemini-1.5-pro-latest", temperature=0.2, max_new_tokens=8192):
        ...
        def sync_logic():
            while True:
                ...
                try:
                    ...
                    generation_config = {"max_output_tokens": max_new_tokens or self.max_new_tokens, "temperature": temperature or self.temperature}
                ...
                with tempfile.NamedTemporaryFile(delete=True, suffix=f"_{file_name}") as temp_f:
                    ...
                    try:
                        ...
                        model_prompt = (
                            ...
                            "Your task is to generate accurate transcribe the audio from the provided file accurately. "
                            "Do not add any commentary, analysis, or introductory phrases. "
                            "Only output the transcribed text."
                        )
                        if language: model_prompt += f" The language is {language}."
